...
function baseLayout(content) {
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ANTI Store</title></head>
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e5e5e5;">
        ...
        <tr><td style="background:#171717;padding:28px 32px;text-align:center;">
          <span style="color:#ffffff;font-size:22px;font-weight:800;letter-spacing:0.12em;">ANTI</span>
        </td></tr>
        <!-- Body -->
        ...
        <tr><td style="background:#fafafa;border-top:1px solid #e5e5e5;padding:20px 32px;text-align:center;">
          <p style="margin:0;font-size:12px;color:#a3a3a3;">ANTI Store &middot; <a href="https://shopanti.online" style="color:#a3a3a3;text-decoration:none;">shopanti.online</a></p>
          <p style="margin:6px 0 0;font-size:11px;color:#d4d4d4;">South Africa</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
function welcomeEmail(name = "there") {
  return baseLayout(`
    <h2 style="margin:0 0 8px;font-size:20px;font-weight:700;color:#171717;">Welcome to ANTI</h2>
    <p style="margin:0 0 20px;font-size:15px;color:#525252;line-height:1.6;">Hi ${escapeHtml(name)}, thanks for creating an account. You're now part of the ANTI community.</p>
    <p style="margin:0 0 24px;font-size:14px;color:#737373;line-height:1.6;">Explore our latest collections and shop pieces that move with you.</p>
    <p style="text-align:center;">${btn("Shop Now", (process.env.FRONTEND_URL || "https://shopanti.online") + "/shop")}</p>
  `);
}

function orderConfirmationEmail(order) {
  const frontendUrl = process.env.FRONTEND_URL || "https://shopanti.online";
  ...
  return baseLayout(`
    ...
    <p style="text-align:center;">${btn("View Order", `${frontendUrl}/checkout/success?custom_str1=${encodeURIComponent(order?.orderNumber || "")}`)}</p>
  `);
}
...
function adminNewOrderEmail(order) {
  ...
  return baseLayout(`
    ...
    <p style="text-align:center;">${btn("Manage Order", (process.env.FRONTEND_URL || "https://shopanti.online") + "/admin")}</p>
  `);
}
...
function resetPasswordEmail(name = "there", resetUrl) {
  return baseLayout(`
    ...
    <p style="margin:0 0 20px;font-size:15px;color:#525252;line-height:1.6;">Hi ${escapeHtml(name)}, we received a request to reset your ANTI Store password. Click below to set a new one — this link expires in <strong>1 hour</strong>.</p>
    <p style="text-align:center;margin:0 0 24px;">${btn("Reset Password", resetUrl)}</p>
    <p style="font-size:13px;color:#a3a3a3;line-height:1.6;">If you didn't request this, you can safely ignore this email. Your password won't change until you click the link above.</p>
  `);
}
...
function adminCustomOrderEmail(inquiry) {
  return baseLayout(`
    ...
    <p style="text-align:center;">${btn("View Admin", (process.env.FRONTEND_URL || "https://shopanti.online") + "/admin")}</p>
  `);
}
